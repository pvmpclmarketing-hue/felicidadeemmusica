...
type CheckoutInput = { recipient?: string; style?: string; name?: string; story?: string; buyerName?: string; buyerPhone?: string };
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const paymentResponse = await fetch(`${asaasUrl}/payments`, { method: "POST", headers, body: JSON.stringify({ customer: customer.id, billingType: "PIX", value: 39.9, dueDate, description: `Canção Única — 2 versões para ${input.name}`, externalReference: order.id }) });
